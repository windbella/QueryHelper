...
public static string BuildUpdate(DataTable sourceDataTable, DataTable targetDataTable, IEnumerable<string> exceptedColumn = null)
{
    ...
    if (UseTrim)
    {
        foreach (DataRow dataRow in tempSource.Rows)
        {
            foreach (DataColumn dataColumn in tempSource.Columns)
            {
                if (dataColumn.DataType == typeof(string))
                {
                    dataRow[dataColumn.ColumnName] = dataRow[dataColumn.ColumnName].ToString().Trim();
                }
            }
        }
        foreach (DataRow dataRow in tempTarget.Rows)
        {
            foreach (DataColumn dataColumn in tempTarget.Columns)
            {
                if (dataColumn.DataType == typeof(string))
                {
                    dataRow[dataColumn.ColumnName] = dataRow[dataColumn.ColumnName].ToString().Trim();
